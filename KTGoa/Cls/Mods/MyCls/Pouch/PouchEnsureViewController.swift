//
//  PouchEnsureViewController.swift
//  Golaa
//
...
class PouchEnsureViewController: BaseCollectionViewController {
    ...
    private func configViews() {
        navTitle(PouchContentType.ensure.rawValue)
    ...
    override func loadListData() {
        super.loadListData()
        MineReq.tradeLogDetail(.ensure, page: page, size: size) { [weak self] list, model, isSuccess, errorMsg in
    ...
    override func registCell(_ collection: UICollectionView) {
        ...
        collection.register(PouchEnsureHeader.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "PouchEnsureHeader")
...
extension PouchEnsureViewController {
    // 手动计算高度Cell高度
...
extension PouchEnsureViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if section == 0 {
            return CGSize(width: screW, height: PouchHeader.ensureContentHeight)
        }
        return .zero
    }
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        if kind == UICollectionView.elementKindSectionHeader {
            let cell = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "PouchEnsureHeader", for: indexPath)
            if let cell = cell as? PouchEnsureHeader {
