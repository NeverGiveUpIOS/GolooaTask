...
open class JRefreshStateHeader: GTRefreshHeader {
    ...
    override var lastUpdatedTimeKey: String? {
        set(newValue) {
            ...
            if lastUpdatedTime != nil {
                ...
                // 3.显示日期
                lastUpdatedTimeLabel.text = String(format: "%@%@%@", JRefreshHead.lastTimeText, isToday ? JRefreshHead.dateTodayText : "", time)
            } else {
                lastUpdatedTimeLabel.text = String(format: "%@%@", JRefreshHead.lastTimeText, JRefreshHead.noneLastDateText)
            }
...
extension JRefreshStateHeader {
    override open func prepare() {
        ...
        // 初始化文字
        setTitle(JRefreshHead.idleText, .Idle)
        setTitle(JRefreshHead.pullingText, .Pulling)
        setTitle(JRefreshHead.refreshingText, .Refreshing)
    }
